"""Re-roda só a avaliação dos 6 modelos finais (DQN) usando os .zip já treinados, sem
...
Sobrescreve outputs/modelos_finais_dqn.csv com as métricas corrigidas.
"""
import os
import sys
import csv
...
def reavaliar(nome_recompensa, com_optuna):
    ...
    sumo_env = criar_sumo_compartilhado(
        out_csv_name=f'outputs/reavaliacao_dqn_{tag}_{nome_recompensa}_eval',
        reward_fn=reward_fn,
        min_green=min_green,
    )
...
if __name__ == "__main__":
    ...
    caminho = 'outputs/modelos_finais_dqn.csv'
